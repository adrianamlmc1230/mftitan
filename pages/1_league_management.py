...
with st.expander("🔄 賽季轉換"):
    leagues_for_rotate = store.list_leagues(active_only=True)
    if not leagues_for_rotate:
        st.info("沒有啟用的聯賽")
    else:
        rotate_options = {f"{lg.code} - {lg.name_zh}": lg for lg in leagues_for_rotate}
        selected_rotate = st.selectbox("選擇聯賽", list(rotate_options.keys()), key="rotate_sel")
        lg = rotate_options[selected_rotate]

        current, previous = store.get_current_previous_pair(lg.id)

        st.markdown("**目前狀態：**")
        if current:
            st.caption(f"本季：{current.label}（id={current.id}）")
        else:
            st.caption("本季：無")
        if previous:
            st.caption(f"上季：{previous.label}（id={previous.id}）")
        else:
            st.caption("上季：無")

        st.markdown("**轉換預覽：**")
        if current and previous:
            st.caption(f"• 現有上季「{previous.label}」→ 封存（role=NULL，資料保留）")
        if current:
            st.caption(f"• 現有本季「{current.label}」→ 變為上季")
        st.caption("• 建立新的本季賽季")

        with st.form("rotate_form"):
            new_label = st.text_input("新賽季標識", placeholder="例：2027第一階段")
            new_year = st.number_input("起始年份", value=2027, min_value=2020, max_value=2050)
            new_phase = st.text_input("階段（選填）", placeholder="第一階段")

            if st.form_submit_button("執行賽季轉換", type="primary"):
                if not new_label.strip():
                    st.error("請輸入新賽季標識")
                else:
                    try:
                        new_sid, needs_confirm = store.rotate_season(
                            lg.id, new_label.strip(), new_year,
                            phase=new_phase.strip() or None,
                            force_overwrite=True,
                        )
                        store.log_action("rotate_season", "league", lg.id,
                                         f"新賽季: {new_label.strip()}, id={new_sid}")
                        st.success(f"賽季轉換完成，新賽季 ID: {new_sid}")
                        st.rerun()
                    except Exception as e:
                        st.error(f"轉換失敗：{e}")
